...
app.get('/node/:id', async function (req: Request, res: Response) {
  ...
  const node = await myDataSource.getRepository(Nodes).findOneBy({ id })
...
app.post('/property', async function (req: Request, res: Response) {
  ...
  if (node == null) return res.json({ error: 'Node not found' })

  const existingProperty = await myDataSource.getRepository(Property).findOneBy({ nodeId, key })

  if (existingProperty != null) return res.status(400).json({ error: 'Duplicated entry for Node-Property' })
